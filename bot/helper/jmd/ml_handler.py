...
from bot import (IS_PREMIUM_USER, LOGGER, bot, categories_dict,
                 config_dict)
from bot.helper.jmd.utils.utils import (get_content_type, is_gdrive_link,
                                            is_magnet, is_mega_link,
                                            is_rclone_path, is_telegram_link,
                                            is_url, new_task, sync_to_async, getGdriveIdFromUrl)
from bot.helper.jmd.other.exceptions import DirectDownloadLinkException
from bot.helper.jmd.other.texts import MIRROR_HELP_MESSAGE
from bot.helper.jmd.utils.j_utils import none_admin_utils, stop_duplicate_tasks
from bot.helper.jmd.manager.task_listener import MirrorLeechListener
from bot.helper.jmd.aria2.aria2_engine import add_aria2c_download
from bot.helper.jmd.utils.direct_link_generator import direct_link_generator
from bot.helper.jmd.rclone.rclone_download import add_rclone_download
from bot.helper.pyrogram_helper.pyrogram_download import TelegramDownloadHelper
from bot.helper.jmd.rclone.rclone_list import RcloneList
from bot.helper.other.commands import Commands
from bot.helper.pyrogram_helper.filters import CustomFilters
from bot.helper.pyrogram_helper.message_utils import (anno_checker, delete_links,
                                                      editMessage,
                                                      get_tg_link_content,
                                                      isAdmin, isBot_canDm,
                                                      open_category_btns,
                                                      ...
                                                      sendLogMessage,
                                                      sendMessage)
from bot.helper.jmd.utils.bulk_links import extract_bulk_links
...
async def _mirror_leech(client, message, isZip=False, extract=False, isQbit=False, isLeech=False, sameDir=None, bulk=[]):
    ...
    if drive_id and is_gdrive_link(drive_id):
        drive_id = getGdriveIdFromUrl(drive_id)
    ...
    #         gmsg = f"Use /{Commands.CloneCommand} to clone Google Drive file/folder\n\n"
    #         gmsg += f"Use /{Commands.ZipMirrorCommand[0]} to make zip of Google Drive folder\n\n"
    #         gmsg += f"Use /{Commands.UnzipMirrorCommand[0]} to extracts Google Drive archive folder/file"
    #         await delete_links(message)
