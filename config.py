...
if len(CONFIG_FILE_URL) and str(CONFIG_FILE_URL).startswith('http'):
    LOGGER.info(f"Downloading Config File: {CONFIG_FILE_URL}")
...
if exists('config.env'):
    ...
    LOGGER.error("Config File is missing! Exiting now")
    exit(1)


RCLONE_CONFIG_URL = environ.get('RCLONE_CONFIG_URL', '')
if len(RCLONE_CONFIG_URL) and str(RCLONE_CONFIG_URL).startswith('http'):
    LOGGER.info(f"Downloading Rclone Config File: {RCLONE_CONFIG_URL}")
    dw_file_from_url(RCLONE_CONFIG_URL, 'rclone.conf')
