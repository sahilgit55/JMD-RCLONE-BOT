...
from bot.helper.jmd.utils.utils import (MirrorStatus, bt_selection_buttons,
...
from bot.helper.pyrogram_helper.message_utils import (anno_checker, isAdmin,
                                                      request_limiter,
                                                      sendMessage,
                                                      sendStatusMessage)
...
async def select(client, message):
    ...
    if len(msg) > 1:
        ...
        msg = ("Reply to an active /{cmd} which was used to start the qb-download or add gid along with cmd\n\n"
               ...
               .format_map({'cmd': Commands.BtSelectCommand, 'mir': Commands.MirrorCommand[0]}))
...
bot.add_handler(MessageHandler(select, filters=command(
    Commands.BtSelectCommand) & CustomFilters.authorized))
